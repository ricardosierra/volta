# Créditos e licenças de assets

Todo asset de terceiros usado no jogo precisa estar listado aqui, com licença e link.
Asset sem entrada **reprova o CI**.

## Fontes

| Asset | Autor | Licença | Uso | Fase |
|---|---|---|---|---|
| _(nenhuma ainda — a fonte definitiva entra em GSD 08 / ART-001)_ | | | | |

## Áudio

| Asset | Autor | Licença | Uso | Fase |
|---|---|---|---|---|
| _(nenhum ainda — GSD 09 / FEEL-006)_ | | | | |

## Outros

| Asset | Autor | Licença | Uso | Fase |
|---|---|---|---|---|
| `brand/volta_app_icon.png` | Gerado com Google Flow, conta do projeto | Uso próprio; saída gerada por IA | ícone da aplicação | Store kit |
| `store/volta_feature_graphic.png` | Gerado com Google Flow, conta do projeto | Uso próprio; saída gerada por IA | feature graphic da loja | Store kit |

## Bibliotecas

| Biblioteca | Licença | Uso | Fase |
|---|---|---|---|
| [GUT](https://github.com/bitwes/Gut) | MIT | testes, apenas em contexto de desenvolvimento | GSD 01 |
